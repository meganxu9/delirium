'''
Base code from “cmu_112_graphics.py” from 15112 course,m odified to include my
own right clicking function, which is binded to <Button-2>. This binds to 
right-click on my Apple computer + mouse, but Windows/other users may need to
bind to <Button-3> instead by editing the number on line 623 in this file.
'''
...
class App(object):
    ...
    def _rightMousePressedWrapper(app, event):
        ...
        if ((event.x < 0) or (event.x > app.width) or
            ...
            app._lastRightMousePosn = (event.x, event.y)
    ...
    def _sizeChangedWrapper(app, event=None):
        ...
        if (app._rightMousePressedOutsideWindow): return
    ...
    def _mouseMotionWrapper(app):
        ...
        if ((not app._paused) and
            ...
            (not app._rightMousePressedOutsideWindow) and 
    ...
    def run(app):
        ...
        if (App._theRoot is None):
            ...
            App._theRoot.bind("<Button-2>", lambda event: App._theRoot.app._rightMousePressedWrapper(event))
...
class ModalApp(App):
    ...
    def rightMousePressed(app, event):
        if (app._activeMode != None): app._activeMode.rightMousePressed(event)
